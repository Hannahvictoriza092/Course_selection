...
QJsonObject CourseAlgorithm::genSimSchedule(const QJsonObject &courseData, int creditLimit) //最终课程安排只是，依赖于总学分
{
    ...
    semesterCourses.assign(8, QVector<ClassInfo>());
    totalCredits = 0;
...
bool CourseAlgorithm::initializeCourseData(const QJsonObject &courseData) //处理courseMap、classMap，所有的信息已经填充上车了
{
    ...
    for (const auto &courseVal : courses) {//单个课程
        ...
        QString semesterStr = courseObj["semester"].toString().toLower();
            ...
            for (QString prereq_id :courseMap[courseID].prerequisites){//前驱结束
                ...
                if(all_pre_ok==false || min_semester>8) {qWarning()<<"前驱没有被上";continue;}//先修课未完，或者超出学期的就是可以滚
                 ...
                 for (int semester=min_semester; semester<8;semester++){
                        bool sem_is_arranged=false;
                        //检查学期开课季节（Autumn/Spring）
                           
                            qWarning()<<courseMap[courseID].name<<"是第"<<courseMap[courseID].semester<<"学期，当前是"<<semester;
                            if (semester % 2 !=courseMap[courseID].semester)//不同，就跳过
                                continue;
                    //遍历课程的所有班级
                    ...
                    for( ClassInfo eachClass:classMap[courseID]){
                          ...
                          //加入课程
                         semesterCourses[semester].append(eachClass);

                         //建立映射
                         Course_in_sem[courseID]=semester;
                         total_credits += courseMap[courseID].credit;
                         qWarning()<<"成功加入"<<courseMap[courseID].name<<" 老师："<<eachClass.teacher<<"课程id"<<classID;
                         sem_is_arranged=true;          
                         break;
                    }
                    if(sem_is_arranged) break;
            }
    ...
    for (int sem = earliestSem; sem < 8; sem += 2) {
        for (const ClassInfo &classInfo : classMap[courseId]) {
            if (!hasTimeConflict(sem, classInfo)) {
                // 做出选择
                semesterCourses[sem].append(classInfo);
...
int CourseAlgorithm::calculateTotalCredits() //所有课的学分
{
    int total = 0;
    for (int i=0;i<8;i++) {
        auto semester=semesterCourses[i];
        for (const ClassInfo & cls : semester) {
            if (courseMap.contains(cls.courseID)) {
                total += courseMap[cls.courseID].credit;
            }
        }
    }
    return total;
...
QJsonObject CourseAlgorithm::buildScheduleJson() //建立新的Jsond对象
{
    QJsonObject result;
    QJsonArray scheduleArray;
    for (int semester =0; semester <8; ++semester) {
        for (const auto &ThisClass : semesterCourses[semester]) { //每学期的课
        //    if (!courseMap.contains(courseId)) continue; //避免课程错误
            ...
            QJsonArray jsonArray;
             ...
             qWarning()<<"选课结果"<<scheduleItem["teacher"]<<" "<<scheduleItem["name"];
...
bool  CourseAlgorithm:: hasTimeConflict(int semester, const ClassInfo &newClass) { //判断这个课程的有没有可以加入不冲突的课程
    for (ClassInfo &existingClass : semesterCourses[semester]) {//对于当前已经放进当前学期的
        // 1. 检查周次是否重叠
        ...
        // 2. 检查具体时间是否重叠
        if(existingClass.times.isEmpty()){qWarning()<<"排课时间为空这是怎么回事";}
